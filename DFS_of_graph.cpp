...
*/
void DFS ( list <int> lists[] ,int vertex , int visited[])
{
        ...
        if(visited[vertex]==0)
        {
                ...
                for(list<int> ::iterator it=lists[vertex].begin();it != lists[vertex].end();it++)
                {
                        if(visited[*it]==0)
                        {
                        DFS(lists,*it,visited);
                        }
...
*/
void DFS_visited(list<int> lists[] , int vertices)
{
...
int main()
{
        ...
        DFS_visited(lists,vertices);  /* DFS function for printing DFS*/
